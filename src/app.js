...
function detail() {
  ...
  element.innerHTML = `<div class="detail-head"><div><div class="eyebrow">FICHA DEL ESTUDIANTE</div><h2>${escape(student.full_name)}</h2><p>${escape(student.group_name)} · Lista ${student.list_number}</p></div><button id="edit-student" class="secondary">Editar datos</button></div><div class="info"><span><small>Cédula</small>${escape(student.document_number)}</span><span><small>Nacimiento</small>${escape(student.birth_date?.split('-').reverse().join('/') || 'Sin dato')}</span><span><small>Código de grupo</small>${escape(student.group_code || 'Sin dato')}</span></div><form id="card" class="document ${tone(status(student))}"><div class="doc-title"><h3>Carné del adolescente</h3>${badge(status(student))}</div><div class="doc-fields"><label>Situación<select name="card_status"><option value="unknown" ${student.card_status === 'unknown' ? 'selected' : ''}>Sin confirmar</option><option value="no_card" ${student.card_status === 'no_card' ? 'selected' : ''}>No tiene</option><option value="has_card" ${student.card_status === 'has_card' ? 'selected' : ''}>Tiene carné</option></select></label><label>Vencimiento (mes y año)<input name="expiry" type="month" inputmode="numeric" pattern="[0-9]{4}-(0[1-9]|1[0-2])" min="2000-01" max="2100-12" value="${escape(student.card_expiry_month?.slice(0, 7) || '')}"></label><button>Guardar</button></div><label>Observaciones<textarea name="notes" rows="3" maxlength="500">${escape(student.notes || '')}</textarea></label><label class="check"><input name="review_required" type="checkbox" ${student.review_required ? 'checked' : ''}>Requiere revisión contra la documentación original</label><p class="muted">Vencimiento registrado: ${expiryLabel(student.card_expiry_month)}</p></form>`;
  document.querySelector('#edit-student').onclick = () => studentModal(student);
  const form = document.querySelector('#card');
  const syncExpiry = () => { const enabled = form.elements.card_status.value === 'has_card'; form.elements.expiry.disabled = !enabled; form.elements.expiry.required = enabled; form.elements.expiry.setCustomValidity(''); if (!enabled) form.elements.expiry.value = ''; };
  form.elements.card_status.onchange = syncExpiry; form.elements.expiry.oninput = () => form.elements.expiry.setCustomValidity(''); syncExpiry();
  form.onsubmit = event => {
    event.preventDefault();
    if (form.elements.card_status.value === 'has_card' && !/^\d{4}-(0[1-9]|1[0-2])$/.test(form.elements.expiry.value)) { form.elements.expiry.setCustomValidity('Selecciona un mes y un año válidos.'); form.elements.expiry.reportValidity(); return; }
    const row = { card_status: form.elements.card_status.value, card_expiry_month: form.elements.card_status.value === 'has_card' && form.elements.expiry.value ? `${form.elements.expiry.value}-01` : null, review_required: form.elements.review_required.checked, notes: form.elements.notes.value.trim() || null };
    action(event.submitter, async () => {
